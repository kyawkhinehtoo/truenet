  ...
  <app-layout>
    ...
    <div class="py-2">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg" v-if="voips.data">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="row in voips.data " v-bind:key="row.id">
                <td class="px-6 py-3 whitespace-nowrap">{{ row.id }}</td>
                <td class="px-6 py-3 whitespace-nowrap">{{ row.voip_number }}</td>
                    ...
                    <div class="">
                      <div class="mb-4">
                        ...
                        <input type="text"
                          ...
                          id="voip_number" placeholder="Enter Voip Name" v-model="form.voip_number">
                        <div v-if="$page.props.errors.voip_number" class="text-red-500">{{
                          $page.props.errors.voip_number[0]
                        }}</div>
        ...
        <span v-if="voips.links">
          <pagination class="mt-6" :links="voips.links" />
        </span>
      </div>
    </div>
  </app-layout>
...
export default {
  ...
  setup(props) {
    ...
    function resetForm() {
      form.voip_number = null
    }
    function submit() {
      if (!editMode.value) {
        form._method = 'POST';
        router.post('/voip', form, {
          preserveState: true,
          onSuccess: (page) => {
            closeModal()
            resetForm()
    ...
    function edit(data) {
      form.id = data.id
      form.voip_number = data.voip_number
      editMode.value = true
      openModal()
    }
    ...
    const searchTsp = () => {
      ...
      router.get('/voip/', { voip: search.value }, { preserveState: true })
    }
